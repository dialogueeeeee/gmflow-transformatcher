...
from .geometry_match import Geometry
from .backbone_match import resnet50, resnet101
...
class Match2Match(nn.Module):
    ...
    def __init__(self, feat_dims,luse):
        ...
        self.posenc = nn.Parameter(torch.randn(12,16,12,16,input_dim), requires_grad=True)

        layer_pos_emb = RotaryEmbedding(dim=4, freqs_for = 'pixel')

        self.to_original = nn.Sequential(
            nn.Linear(input_dim, 1),
            Rearrange('b (h1 w1 h2 w2) c -> b c h1 w1 h2 w2', h1=12, w1=16, h2=12, w2=16),
            # Rearrange('b (h1 w1 h2 w2) c -> b c h1 w1 h2 w2', h1=23, w1=31, h2=23, w2=31),
        )
        ...
        for _ in range(layer_num):
            self.trans_nc.append(nn.ModuleList([
                ...
                PreNorm(input_dim, FastAttention(input_dim, heads = 4, dim_head = 4, pos_emb=layer_pos_emb)),
    ...
    def forward(self, src_feats, trg_feats):
        ...
        bsz, ch, ht, wd, _, _ = correlations.size()
        ...
        correlations = Geometry.interpolate4d(refined_corr.squeeze(1)).unsqueeze(1)

        # side = correlations.size(-1) ** 2
        # correlations = correlations.view(bsz, side, side).contiguous()

        _, _, hf, wf, _, _ = correlations.size()
        # correlations = correlations.view(bsz*hf*wf, 1, hf, wf).contiguous()
        correlations = correlations.view(bsz, hf*wf, hf*wf).contiguous()
...
class TransforMatcher(nn.Module):
    ...
    # def __init__(self, backbone, luse, device, ht, wd):
    def __init__(self, backbone, luse, ht, wd):
        ...
        with torch.no_grad():
            # dummy = torch.randn((2, 3, ht, wd)).to(device)
            dummy = torch.randn((2, 3, ht, wd))
        ...
        self.match2match = Match2Match(feat_dim, luse)
    ...
    # def forward(self, src_feats, trg_feats):
        ...
        return correlation_ts, src_feats, trg_feats
