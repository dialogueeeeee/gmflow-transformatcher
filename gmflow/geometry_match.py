...
class Geometry(object):
    ...
    def interpolate4d(tensor4d):
        ...
        tensor4d = F.interpolate(tensor4d, [hup, wup], mode='bilinear', align_corners=True)
        tensor4d = tensor4d.view(bsz, h2, w2, -1).permute(0, 3, 1, 2)
        tensor4d = F.interpolate(tensor4d, [hup, wup], mode='bilinear', align_corners=True)
        tensor4d = tensor4d.view(bsz, hup, wup, hup, wup)
        return tensor4d
    ...
    def cosine_similarity(cls, src_feats, trg_feats):
        correlations = []
        bsz, _, ht, wd = src_feats[len(src_feats)-1].size() 

        for src_feat, trg_feat in zip(src_feats, trg_feats):
            _, nch, _, _ = src_feat.size()

            src_feat = l2normalize(src_feat, dim=1)
            trg_feat = l2normalize(trg_feat, dim=1)
            src_feat = F.interpolate(src_feat, [ht, wd], mode='bilinear', align_corners=True)
            trg_feat = F.interpolate(trg_feat, [ht, wd], mode='bilinear', align_corners=True)

            src_feat = src_feat.view(bsz, nch, -1).transpose(1, 2)
            trg_feat = trg_feat.view(bsz, nch, -1)
            corr = torch.bmm(src_feat, trg_feat)
            correlations.append(corr.view(bsz, 1, ht, wd, ht, wd))
        return correlations
